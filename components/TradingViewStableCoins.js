...
const STABLECOIN_SYMBOLS = [
    { name: 'BINANCE:USDPUSDT', displayName: 'Pax Dollar' },
    { name: 'BINANCE:USDCUSDT', displayName: 'USD Coin' },
    { name: 'BINANCE:FDUSDUSDT', displayName: 'First Digital USD' },
    { name: 'BINANCE:TUSDUSDT', displayName: 'TrueUSD' },
    { name: 'BINANCE:PYUSDUSDT', displayName: 'PayPal USD' },
    { name: 'BINANCE:USDDUSDT', displayName: 'USDD' },
];

function TradingViewWidget({ symbol, isDarkMode, index }) {
    const containerRef = useRef(null);
    const widgetId = `tradingview-quote-stable-${symbol.replace(/[^a-zA-Z0-9]/g, '-')}-${index}`;
...
export default function TradingViewStableCoins({ isDarkMode = true }) {
    return (
        <div className={`h-full w-full overflow-y-auto overflow-x-hidden p-3 custom-scrollbar transition-colors ${isDarkMode ? 'bg-black/20' : 'bg-white'}`}>
            {STABLECOIN_SYMBOLS.map((sym, index) => (
